...
export async function createUser(db: DB, user: InferInsertModel<typeof users>, platformId: string, authToken: string) {
	...
	await db.batch([
		...
		db.insert(platformUsers).values({ userId: newId, platform: Platform.Telegram, platformId: platformId }),
	]);
}

export async function createPlatformUser(db: DB, id: string, platformId: string) {
	await db.insert(platformUsers).values({ userId: id, platform: Platform.Telegram, platformId: platformId });
}
